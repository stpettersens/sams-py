...
namespace Yedda {
	public class Twitter {
		...
		public enum ObjectType {
			...
			Account,
			Users,
            IDs // added for GetFollowersForUser method
		}
        ...
        /*
            NEW METHOD to get followers for a specific user
        */
        public string GetUserFollowers(string IDorScreenName, OutputFormatType format) {
            if (format != OutputFormatType.JSON && format != OutputFormatType.XML) {
                throw new ArgumentException("GetUserFollowers supports only XML and JSON output format", "format");
            }

            string url = string.Format(TwitterBaseUrlFormat, GetActionTypeString(ActionType.Followers), GetObjectTypeString(ObjectType.IDs) + "/" + IDorScreenName, GetFormatTypeString(format));
            return ExecuteGetCommand(url, "", ""); // Do not authenticate
        }

        public string GetUserFollowersAsJSON(string IDorScreenName) {
            return GetUserFollowers(IDorScreenName, OutputFormatType.JSON);
        }
